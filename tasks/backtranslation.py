...
class NLLBDataset(CollatedDataset):
    ...
    def __init__(
        ...
        if src_lang < tgt_lang:
            assert dataset["source_lang"][0] in src_lang  # assuming src_lang is given in FLORES format
            assert dataset["target_lang"][0] in tgt_lang
            self.src_texts, self.tgt_texts = dataset["source_text"], dataset["target_text"]
        else:
            assert dataset["target_lang"][0] in src_lang
            assert dataset["source_lang"][0] in tgt_lang
...
class ParaphraseEval:
    ...
    def __init__(self, device: str, model_id: str = "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"):
        from sentence_transformers import SentenceTransformer

        self.encoder = SentenceTransformer(model_id, device=device)
    ...
    def compute_similarity(self, texts1: List[str], texts2: List[List[str]]) -> List[List[float]]:
        ...
        #  - Marek thinks it's not a good idea as it would spoil the "all bad" batches
...
class Backtranslation(Task):
    ...
    def __init__(self,
        ...
        # TODO: clean up model config and parametrize
        target_lang_m_id = "Mike0307/multilingual-e5-language-detection"
        self.target_lang_m = transformers.AutoModelForSequenceClassification.from_pretrained(target_lang_m_id)
        self.target_lang_m = self.target_lang_m.to(device)
        self.target_lang_index = 8
        self.target_lang_m_tokenizer = transformers.AutoTokenizer.from_pretrained(target_lang_m_id)

        self.sim_model = ParaphraseEval(device)

        # TODO: resolve FLORES as eval dataset(?)
    ...
    def _target_lang_reward(self, generated_strs: List[List[str]]) -> List[List[float]]:
        # TODO: this will likely need to be contextualized:
        #  - but maybe it's done by the algorithm itself -- we should check that. If not:
        #  - rescale on the whole batch of outputs
        #  - normalize with probs of true sequence -> not happening, because that would be the use of target sequences
        device = self.target_lang_m.device
        bs = len(generated_strs[0])
        ...
        batch_inputs = self.target_lang_m_tokenizer(generated_strs_flat, return_tensors="pt", padding=True,
                                                    truncation=True, max_length=512).to(self.target_lang_m.device)
        # infer logits, normalize, shift labels, slice labels' probs, aggregate, normalize
    ...
    def reward_responses(self,
        ...
        for batch_i, (batch_input_str, batch_input_ids) in enumerate(batch_iter_args):
            ...
            sample_iter_args = zip(generated_strs[batch_i], generated_encodings[batch_i], lm_rewards[batch_i], bt_rewards[batch_i])
